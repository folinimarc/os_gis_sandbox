...
software like ArcGIS were ignored. Table \ref{tab:t_content_analysis} shows the
...
\subsubsection*{Idea 1: Sandbox as direct installation}
It was found that only 3 established open-source projects (QGIS, PostGIS and
Geoserver) would probably be enough to satisfy the functional requirements. The
roles of PostGIS as spatial database and Geoserver as OGC data server are
well-defined. The QGIS installation is particularly interesting because it comes
bundled with versions of GDAL and python that work together well and provides
appropriate interfaces.

The community has produced installers for all major
operating systems, which makes the installation relatively straight-forward.
