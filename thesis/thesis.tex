...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    to fully focus on how the different tools interact with the data and other
    tools to achieve a meaningful result. This is in contrast to the majority of
    tutorials, which are tool focused and to which data is secondary to teach
    the necessary functionalities. An example data story was implemented whereby
    ...
    learn about the GDAL command line tools to inspect datasets and load them
    into PostGIS, where spatial processing and aggregation is performed. The
    result is made available as a new dataset to be consumed and styled in QGIS.
  \end{abstract}
\end{titlepage}
...
a spatial database and an OGC data server as containers isolated from the host
...
and GDAL and python scripting capabilities with access to libraries from the
